...
def __plot_sig_label(x1,x2,y,text,ax=None,fontoffset=0,linewidth=1.5,font_size=None):
...
def __plot_sig_labels_hue(P_values,order,y0,deltay,ax,x_offset,show_ns=True,width=0.8,
                        use_stars=True,labelkws=None):
...
def ___plot_sig_labels_xaxis(P_values,order,y0,deltay,
...
def plot_all_sig_labels(P_values,order,order_hue=None,show_ns=False,
                        y0='auto',deltay='auto',
                        ax=None,**kws):
    ...
    if order_hue is None:
        ___plot_sig_labels_xaxis(P_values,order,ax=ax,
    ...
    # plot sig labels usign hue
        ...
        for cat,P in  P_values.groupby(level= 0):
                ...
                __plot_sig_labels_hue(P_values.loc[cat],order,ax=ax,
